...
const std::vector<double> &vKZ)
{
	...
	std::cout << "WI = " << mWells[0].WI << std::endl;
	std::cout << "KH = " << Kh << std::endl;
	mWells[2].is_producer = false;
	mWells[2].loc = 210;
	mWells[2].WI = 0.001127 * 6.2832 * Kh / log(ro / 0.5);
	mWells[2].QINJ[PhaseID::W] = 300.0;
	mWells[2].QINJ[PhaseID::O] = 0.0;
	...
	for (std::size_t f = 0; f < mMesh.size_faces(); ++f)
	{
		...
		for (std::size_t ph = 0; ph < 2; ++ph)
		{
			mFaces[f].Pot[ph] = (mCells[c2].P[ph] - mCells[c1].P[ph]) +
				0 * 0.5 * (mCells[c2].Rho[ph] + mCells[c1].Rho[ph]) * dz;
	...
	for (std::size_t w = 0; w<mWells.size(); ++w)
	{
		...
		if (mWells[w].is_producer)
		{
			...
			//	 if ( mTmpVars[ 0 ].value() >= 0.0 )
			{
				...
				//adetl::ADscalar<> n=mWells[w].WI * mTmpVars[0] * mCells[c].Kr[PhaseID::O] * mCells[c].bmu[PhaseID::O];
				//std::cout << n.value() << "  " << c << std::endl;
				mFlow[eqnID(c, PhaseID::W)] += mWells[w].WI * mTmpVars[0] * mCells[c].Kr[PhaseID::W] * mCells[c].bmu[PhaseID::W];
				//adetl::ADscalar<> m = mWells[w].WI * mTmpVars[0] * mCells[c].Kr[PhaseID::W] * mCells[c].bmu[PhaseID::W];
			}
		}
		else
		{
