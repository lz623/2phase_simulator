...
public:
   ...
   template< typename T >
   void calculate_f(double PHI, const T & _state, CellProps & p)
   {
	   p.status = _state.status;
	   p.P[O] = _state.Po;
	   p.S[W] = _state.Sw;
...
private:
   ...
   void calc_rock_fluid_f(CellProps &p)
   {
	   std::size_t i = SW_f_key(p.S[W]);
	   KRW_f_val(i, SW_f_key, p.S[W], p.Kr[W]);
	   KRO_f_val(i, SW_f_key, p.S[W], p.Kr[O]);
	   PCOW_f_val(i, SW_f_key, p.S[W], mTmp[0]);
	   p.P[W] = p.P[O] - mTmp[0];
   }
   void
   read_PVDO_table( )
	   ...
	   read_SWOF_f_table()
   {
		   // read SWOF table
		   double tmp;
		   std::ifstream strm("./input/SWOFF.dat");
		   while (strm.good())
		   {
			   if (strm >> tmp) SW_f_key.push_back(tmp);
			   if (strm >> tmp) KRW_f_val.push_back(tmp);
			   if (strm >> tmp) KRO_f_val.push_back(tmp);
			   if (strm >> tmp) PCOW_f_val.push_back(tmp);
		   }
		   strm.close();
	   }
private:
   ...
   TableValue   PCOW_val;
   TableKey		SW_f_key;
   TableValue   KRW_f_val;
   TableValue   KRO_f_val;
   TableValue   PCOW_f_val;
   scalar       mTmp[5];
};
