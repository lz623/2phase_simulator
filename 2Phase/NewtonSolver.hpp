...
namespace GENSOL{
	typedef CSR_Matrix< double, int >        CSR;
  ...
  public:
   ...
   //   if (verbose > 0) 
	...
	report_t solve_timestep_new(typename Model::StateVector &newState,
		const typename Model::StateVector &oldState,
		double DT,
		Model &model,
		LNsolver &lnsolver,
		CSR &Jaco,
		DiscreteProblem::Sparse_matrix &oJaco)
	{
		...
		std::vector<double> old_acc(oldState.size());
		model.extract_acc(oJaco);
		Jaco = J;
		return status;
	}
